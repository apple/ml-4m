...
def simple_span_masking(
    sequence: List[int], sentinel_to_id: Dict[int, int], keep_prob: float
...
def chunk_span_masking(
    sequence_chunks: List[List[int]], sentinel_to_id: Dict[int, int], keep_prob: float
) -> Tuple[List[int], List[int]]:
...
class UnifiedMasking(object):
    ...
    def input_token_budget(self, num_input_tokens, dir_idx=0):
        ...
        for i in range(self.max_tries):
            ...
            input_token_budget += torch.bincount(
                self.input_dirichlets[dir_idx].sample_n(diff).argmax(dim=-1), minlength=len(input_token_budget)
            )
    ...
    def target_token_budget(self, input_token_budget, num_target_tokens, dir_idx=0):
        ...
        max_tokens_remaining = torch.where(
            self.mod_is_img, self.max_tokens - torch.tensor(input_token_budget), self.max_tokens
        )
        ...
        for i in range(self.max_tries):
            ...
            # Adds the remaining tokens by sampling from the Dirichlet and taking the argmax
            # This avoids adding tokens to modalities that shouldn't be sampled (i.e. with alphas ~=0)
            target_token_budget += torch.bincount(
                self.target_dirichlets[dir_idx].sample_n(diff).argmax(dim=-1), minlength=len(target_token_budget)
            )
    ...
    def image_mask(self, tensor: torch.Tensor, num_tokens: int, input_budget: int, target_budget: int):
        ...
        first_mask_token = torch.argmin(
            target_mask + torch.arange(target_mask.shape[0], device=target_mask.device) * 1e-6
        )
    ...
    def sequence_token_mask(
        ...
        if input_budget == 0:
            ...
            if keep_scheme == "random":
                ...
                keep_prob = random.choice([0.0, 1.0])
            else:
        ...
        if target_budget is None or len(target_seq_ids) <= target_budget:
            ...
            if len(target_seq_ids) - sentinel_indices[chosen_sentinel] >= target_budget:
                target_seq_ids = target_seq_ids[
                    sentinel_indices[chosen_sentinel] : sentinel_indices[chosen_sentinel] + target_budget
                ]
    ...
    def sequence_mask(
        self, sequence: Union[str, List[str]], max_tokens: int, input_budget: int, target_budget: int, keep_scheme: str
    ):
        ...
        if isinstance(sequence, str):
            ...
            seq_ids = [
                chunk for (chunk, token_count) in zip(seq_ids, cumulative_token_count) if token_count <= max_tokens
            ]
    ...
    def sequence_emb_mask_span(
        self, emb_tensor: torch.Tensor, max_tokens: int, input_budget: int, target_budget: int, keep_scheme: str
    ):
        ...
        if input_budget == 0:
            ...
            while len(fake_input_seq_ids) > input_budget:
                keep_prob = keep_prob * self.keep_prob_decay_factor
                fake_input_seq_ids, fake_target_seq_ids = simple_span_masking(
                    fake_seq_ids, self.sentinel_to_id, keep_prob
                )
    ...
    def __call__(self, mod_dict):
        ...
        for (mod_name, mod_info), input_budget, target_budget in zip(
            self.modality_info.items(), input_token_budget, target_token_budget
        ):
            ...
            if mod_type == "img":
                masked_mod_dict[mod_name] = self.image_mask(
                    mod_dict[mod_name_load], mod_info["max_tokens"], input_budget, target_budget
                )
            elif mod_type == "seq":
                keep_scheme = "random" if ("keep" not in mod_info) else mod_info["keep"][dir_idx]
                masked_mod_dict[mod_name] = self.sequence_mask(
                    mod_dict[mod_name_load], mod_info["max_tokens"], input_budget, target_budget, keep_scheme
                )
            elif mod_type == "seq_token":
                keep_scheme = "random" if ("keep" not in mod_info) else mod_info["keep"][dir_idx]
                vocab_offset = mod_info.get(
                    "vocab_offset", 0
                )  # Check if any space is allocated to sentinel tokens and other special tokens
    ...
    def __call__(self, mod_dict):
        ...
        for mod_name, mod_info in self.modality_info.items():
            ...
            if mod_type == "img" and mod_name in self.input_modalities:
                ...
                raise ValueError(
                    f"Invalid modality type: {mod_type} or modality name not in input or target modalities: {mod_name}"
                )

...
def setup_sampling_mod_info(dataset_config, modality_info):
    ...
    if dataset_config.get("alphas_config", None) is None:
        for mod in mod_info:
            mod_info[mod]["input_alphas"] = [0.0]
            mod_info[mod]["target_alphas"] = [0.0]
...
def get_train_dataloader(
    ...
    if "caption" in modality_transforms:
        modality_transforms["caption"] = CaptionTransform(aligned_captions=dataset_config.get("aligned_captions", True))
...
def get_val_dataloader(
    ...
    if dataset_type == "multimodal":
        ...
        if is_pretokenized:
            ...
            eval_image_augmenter = CenterCropImageAugmenter(target_size=input_size, main_domain=main_augment_domain)

...
DEFAULTS_RGB2X = {
    ...
    "caption/det": {
        "tokens_per_target": 256,
        "autoregression_scheme": "autoregressive",
        "decoding_steps": None,
        "token_decoding_schedule": None,
        "temp": 0.3,
        "temp_schedule": "constant",
        "cfg_scale": 1.0,
        "cfg_schedule": "constant",
...
DEFAULTS_SR = {
    "tok_clip@448/tok_depth@448/tok_normal@448/tok_semseg@448/tok_rgb@448": {
        "tokens_per_target": 784,
        "autoregression_scheme": "maskgit",
        "decoding_steps": 8,
...
class Demo4MSampler(nn.Module):
    ...
    def __setup_sr_conds_and_targets(self, sample):
        ...
        target_domains_sr = [
            domain
            for domain in DEFAULT_ORDER_SR
            if (domain.replace("448", "224") in cond_domains_sr and domain in self.mods_sr)
        ]
    ...
    def __setup_sample_and_schedule(self, sample, cond_domains, target_domains, cfg_grow_conditioning=True):
        ...
        for cond_mod in sample.keys():
            sample_dict = init_full_input_modality(
                sample_dict, MODALITY_INFO, cond_mod, self.device, eos_id=self.tok_text.token_to_id("[EOS]")
            )
    ...
    def __setup_sr_sample_and_schedule(
        ...
        autoregression_schemes_sr = [
            get_value(DEFAULTS_SR, domain, "autoregression_scheme") for domain in target_domains_sr
        ]
        decoding_steps_sr = [get_value(DEFAULTS_SR, domain, "decoding_steps") for domain in target_domains_sr]
        token_decoding_schedules_sr = [
            get_value(DEFAULTS_SR, domain, "token_decoding_schedule") for domain in target_domains_sr
        ]
        ...
        schedule_sr = build_chained_generation_schedules(
            cond_domains=cond_domains_sr,
            target_domains=target_domains_sr,
            tokens_per_target=tokens_per_target_sr,
            autoregression_schemes=autoregression_schemes_sr,
            decoding_steps=decoding_steps_sr,
            token_decoding_schedules=token_decoding_schedules_sr,
            temps=temps_sr,
            temp_schedules=temp_schedules_sr,
            cfg_scales=cfg_scales_sr,
            cfg_schedules=cfg_schedules_sr,
            cfg_grow_conditioning=cfg_grow_conditioning_sr,
        )
        ...
        for cond_mod in cond_domains_sr:
            sample_sr = init_full_input_modality(
                sample_sr, MODALITY_INFO, cond_mod, self.device, eos_id=self.tok_text.token_to_id("[EOS]")
            )
    ...
    def forward(
        ...
        dec_dict = decode_dict(out_dict, self.toks, self.tok_text, image_size=224, patch_size=16, decoding_steps=50)
        ...
        if self.sampler_fm_sr is not None:
            ...
            sample_sr, generation_schedule_sr = self.__setup_sr_sample_and_schedule(
                out_dict, cond_domains_sr, target_domains_sr
            )
            ...
            dec_dict = decode_dict(
                out_dict_sr, self.toks, self.tok_text, image_size=448, patch_size=16, decoding_steps=50
            )
    ...
    def plot_modalities(self, mod_dict, ncols_max=5, figscale=4.0, save_path=None, use_fixed_plotting_order=False):
        ...
        fig, ax = plt.subplots(
            nrows=nrows, ncols=ncols, figsize=(ncols * figscale, nrows * figscale), facecolor=(1, 1, 1)
        )

        if use_fixed_plotting_order:
            mod_dict = {k: mod_dict[k] for k in MODALITY_PLOTTING_ORDER if k in mod_dict}
        ...
        for i, (mod_name, mod) in enumerate(mod_dict.items()):
            ...
            if mod_name == "det":
                ...
                rgb_background = next(
                    (mod_dict[key] for key in keys_in_order if key in mod_dict), np.ones((224, 224, 3))
                )
                ...
                metadata_pred = ",\n".join(
                    [f"{k}: {v:.2f}" if isinstance(v, float) else f"{k}: {v}" for k, v in mod.items()]
                )

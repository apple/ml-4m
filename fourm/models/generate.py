...
from fourm.utils.generation import (
    cosine_schedule,
    linear_schedule,
    onex_temp_schedule,
    ...
    def forward_mask_encoder_generation(self, encoder_mod_dict):
        ...
        if self.model.num_register_tokens > 0:
            ...
            encoder_mask = torch.cat(
                [torch.zeros((B, prompt_tokens.shape[1]), dtype=torch.bool, device=encoder_mask.device), encoder_mask],
                dim=1,
            )
            mod_mask = torch.cat(
                [torch.full((B, prompt_tokens.shape[1]), -1, dtype=torch.int16, device=mod_mask.device), mod_mask],
                dim=1,
            )

...
class LoRAWrapper(nn.Module):
    ...
    def __init__(self, linear: nn.Module, rank: int = 4, scale: float = 1.0, num_packed_linear: int = 1):
        ...
        assert num_packed_linear * rank <= min(
...
def inject_trainable_LoRA(
    model: nn.Module, rank: int = 4, scale: float = 1.0, target_replace_modules: Set[str] = ATTENTION_MODULES
) -> None:
    ...
    """
    for _module, name, _child_module in _find_modules(model, target_replace_modules, search_class=[nn.Linear]):
...
def fuse_LoRA_into_linear(model: nn.Module, target_replace_modules: Set[str] = ATTENTION_MODULES) -> None:
    ...
    Args:
        ...
        target_replace_modules: Set of module names to replace linear layers in.
    """
    for _module, name, _child_module in _find_modules(model, target_replace_modules, search_class=[LoRAWrapper]):

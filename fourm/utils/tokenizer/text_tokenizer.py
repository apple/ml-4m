...
def generate_sentinel_tokens(num=100, start_id=0):
    tokens = [
        AddedToken(content=f"[S_{i}]", single_word=True, normalized=False) for i in range(start_id, num + start_id)
    ]
...
def generate_object_class_tokens(dataset="coco"):
    ...
    tokens = [AddedToken(content=class_name, single_word=True, normalized=True) for class_name in object_classes]

...
def get_args(args=None):
    ...
    parser.add_argument(
        "--target_domains",
        default="tok_clip-tok_normal-tok_rgb",
        type=str,
    ...
    parser.add_argument(
        ...
        default="maskgit-maskgit-maskgit",
        type=str,
    ...
    parser.add_argument("--rgb_tok_id", default=None, type=str, help="RGB tokenizer ID (default: %(default)s)")
    parser.add_argument("--depth_tok_id", default=None, type=str, help="Depth tokenizer ID (default: %(default)s)")
    parser.add_argument("--normal_tok_id", default=None, type=str, help="Normal tokenizer ID (default: %(default)s)")
    parser.add_argument("--edges_tok_id", default=None, type=str, help="Edges tokenizer ID (default: %(default)s)")
    parser.add_argument("--semseg_tok_id", default=None, type=str, help="Semseg tokenizer ID (default: %(default)s)")
    parser.add_argument("--clip_tok_id", default=None, type=str, help="CLIP tokenizer ID (default: %(default)s)")
    parser.add_argument("--dinov2_tok_id", default=None, type=str, help="DINOv2 tokenizer ID (default: %(default)s)")
    parser.add_argument(
        "--imagebind_tok_id", default=None, type=str, help="ImageBind tokenizer ID (default: %(default)s)"
    )
    parser.add_argument(
        "--dinov2_glob_tok_id", default=None, type=str, help="DINOv2 global tokenizer ID (default: %(default)s)"
    )
    parser.add_argument(
        "--imagebind_glob_tok_id", default=None, type=str, help="ImageBind global tokenizer ID (default: %(default)s)"
    )
    parser.add_argument(
        "--sam_instance_tok_id", default=None, type=str, help="SAM instance tokenizer ID (default: %(default)s)"
    )
    parser.add_argument(
        "--human_poses_tok_id", default=None, type=str, help="Human poses tokenizer ID (default: %(default)s)"
    )
    parser.add_argument(
        "--text_tok_path",
    ...
    parser.add_argument("--controlnet_id", default=None, type=str, help="RGB ControlNet ID (default: %(default)s)")
    parser.add_argument(
    ...
    parser.add_argument("--image_size", default=224, type=int, help="Image size. (default: %(default)s)")
    parser.add_argument(
        "--patch_size", default=16, type=int, help="Base patch size for image-like modalities (default: %(default)s)"
    )
    ...
    # Data
    parser.add_argument(
        "--data_path", default="/mnt/datasets/cc12_multitask_224/val", help="Path to dataset (default: %(default)s)"
    )
    parser.add_argument(
        "--data_name",
        default="",
    ...
    parser.add_argument("--name", default="", type=str, help="wandb and folder name (default: %(default)s)")
    parser.add_argument("--sr_name", default="", type=str, help="SR wandb and folder name (default: %(default)s)")
    parser.add_argument("--output_dir", default="", help="Path where to save, empty for no saving")
    parser.add_argument("--num_log_images", default=100, help="Number of images to log (default: %(default)s)")
    parser.add_argument(
        "--save_all_outputs",
        action="store_true",
    ...
    parser.add_argument(
        "--no_save_all_outputs", action="store_false", dest="save_all_outputs", help="Do not save any outputs."
    )
    ...
    parser.add_argument("--wandb_mode", default="online", type=str, help="Wandb mode")
    parser.add_argument("--show_user_warnings", default=False, action="store_true")

    # GPU / Distributed parameters
    parser.add_argument("--device", default="cuda", help="Device to use for training / testing")
    parser.add_argument("--dist_gen", action="store_true", default=False, help="Enabling distributed generation")
    parser.add_argument("--no_dist_gen", action="store_false", dest="dist_gen", help="Disabling distributed generation")
...
def get_dataset(args, text_tokenizer):
    ...
    # For caption->X generation using Parti Prompts
        ...
        dataset = PartiPromptsDataset(
            text_tokenizer, max_length=128, parti_prompts_t5_embs=args.parti_prompts_t5_embs, llm_embedder=llm_embedder
        )
    ...
    # Otherwise, construct CC12M/IN1K-like pre-tokenized dataset
        ...
        for k in modality_info:
            modality_info[k]["input_alphas"] = [0.0]
            modality_info[k]["target_alphas"] = [0.0]
            modality_info[k]["keep"] = ["all"]

        if "tok" not in "-".join(args.loaded_domains):
            image_augmenter = RandomCropImageAugmenter(
                target_size=args.image_size, hflip=False, crop_scale=(1.0, 1.0), crop_ratio=(1.0, 1.0)
            )
        else:
        ...
        transform = transforms.Compose(
            [
                ...
                UnifiedMasking(
                    ...
                    input_tokens_range=512,
                    target_tokens_range=512,
                ),
            ]
        )
...
def create_superres_input(out_dict, sr_cond_domains, sr_target_domains, sr_tokens_per_target, text_tokenizer, device):
    ...
    for cond_mod in sr_cond_domains:
        superres_sample = init_full_input_modality(
            superres_sample, MODALITY_INFO, cond_mod, device, eos_id=text_tokenizer.token_to_id("[EOS]")
        )
...
def main(args):
    ...
    args.output_dir = os.path.join(
        args.output_dir, args.data_name, f"{args.name}--{args.sr_name}" if args.sr_name else args.name
    )
    ...
    args.tokens_per_target = repeat_if_necessary(
        string_to_list(args.tokens_per_target, dtype=int, delim=delim), n_targets
    )
    args.autoregression_schemes = repeat_if_necessary(
        string_to_list(args.autoregression_schemes, dtype=str, delim=delim), n_targets
    )
    args.decoding_steps = repeat_if_necessary(string_to_list(args.decoding_steps, dtype=int, delim=delim), n_targets)
    args.token_decoding_schedules = repeat_if_necessary(
        string_to_list(args.token_decoding_schedules, dtype=str, delim=delim), n_targets
    )
    ...
    args.sr_tokens_per_target = repeat_if_necessary(
        string_to_list(args.sr_tokens_per_target, dtype=int, delim=delim), sr_n_targets
    )
    args.sr_autoregression_schemes = repeat_if_necessary(
        string_to_list(args.sr_autoregression_schemes, dtype=str, delim=delim), sr_n_targets
    )
    args.sr_decoding_steps = repeat_if_necessary(
        string_to_list(args.sr_decoding_steps, dtype=int, delim=delim), sr_n_targets
    )
    args.sr_token_decoding_schedules = repeat_if_necessary(
        string_to_list(args.sr_token_decoding_schedules, dtype=str, delim=delim), sr_n_targets
    )
    args.sr_temps = repeat_if_necessary(string_to_list(args.sr_temps, dtype=float, delim=delim), sr_n_targets)
    args.sr_temp_schedules = repeat_if_necessary(
        string_to_list(args.sr_temp_schedules, dtype=str, delim=delim), sr_n_targets
    )
    args.sr_cfg_scales = repeat_if_necessary(string_to_list(args.sr_cfg_scales, dtype=float, delim=delim), sr_n_targets)
    args.sr_cfg_schedules = repeat_if_necessary(
        string_to_list(args.sr_cfg_schedules, dtype=str, delim=delim), sr_n_targets
    )
...
def generate(gen_sampler, gen_sampler_sr, tokenizers, text_tokenizer, data_loader, device, dtype, args):
    ...
    if "tok_rgb" in args.target_domains:
        inception_metric = InceptionScore(
            feature="logits_unbiased", splits=10, normalize=False, sync_on_compute=True
        ).to(device)
        ...
        if "caption" in args.cond_domains:
            clip_metric = CLIPScore(model_name_or_path="openai/clip-vit-large-patch14", sync_on_compute=True).to(device)
    ...
    if gen_sampler_sr is not None and "tok_rgb@448" in args.sr_target_domains:
        ...
        if "caption" in args.cond_domains:
            clip_metric_sr = CLIPScore(model_name_or_path="openai/clip-vit-large-patch14", sync_on_compute=True).to(
                device
            )
    ...
    for sample, sample_idx in metric_logger.log_every(data_loader, print_freq=1, header="Generation:"):
        ...
        for cond_mod in args.cond_domains:
            sample = init_full_input_modality(
                sample, MODALITY_INFO, cond_mod, device, eos_id=text_tokenizer.token_to_id("[EOS]")
            )
            ...
            if gen_sampler_sr is not None:
                with torch.cuda.amp.autocast(dtype=dtype, enabled=dtype != torch.float32):
                    sample_sr = create_superres_input(
                        out_dict,
                        args.sr_cond_domains,
                        args.sr_target_domains,
                        args.sr_tokens_per_target,
                        text_tokenizer,
                        device,
                    )
                ...
                if inception_metric_sr is not None:
                    rgb_pred = (
                        TF.to_tensor(255 * dec_dict_sr["tok_rgb@448"]).to(dtype=torch.uint8, device=device).unsqueeze(0)
                    )
                    rgb_pred = TF.resize(rgb_pred, size=args.image_size_metrics)
                    ...
                    if clip_metric_sr is not None:
                        caption_trunc = truncate_caption_for_clip(
                            dec_dict["caption"][0], clip_metric_sr.processor.tokenizer
                        )
        ...
        if args.save_all_outputs:
            save_conds_and_targets(
                args.cond_domains, args.target_domains, dec_dicts, save_dir=args.output_dir, sample_idx=sample_idx
            )

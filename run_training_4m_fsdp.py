...
def get_args():
    ...
    parser.add_argument(
        ...
        default=256,
        type=int,
        help="Batch size per GPU (default: %(default)s). " "Effective batch size is batch_size * accum_iter * # gpus",
    )
    ...
    parser.add_argument(
        "--save_ckpt_freq", default=20, type=int, help="Checkpoint saving frequency in epochs (default: %(default)s)"
    )
    ...
    parser.add_argument(
        "--model",
        default="fm_base_12e_12d_swiglu_nobias",
        type=str,
    ...
    parser.add_argument(
        "--input_size", default=224, type=int, help="Images input size for backbone (default: %(default)s)"
    )
    parser.add_argument(
        "--num_register_tokens",
        default=0,
    ...
    parser.add_argument("--opt", default="adamw", type=str, help="Optimizer (default: %(default)s)")
    parser.add_argument("--opt_eps", default=1e-8, type=float, help="Optimizer epsilon (default: %(default)s)")
    parser.add_argument(
        "--opt_betas", default=[0.9, 0.95], type=float, nargs="+", help="Optimizer betas (default: %(default)s)"
    )
    parser.add_argument("--clip_grad", type=float, default=None, help="Clip gradient norm (default: %(default)s)")
    parser.add_argument("--momentum", type=float, default=0.9, help="SGD momentum (default: %(default)s)")
    ...
    parser.add_argument(
        "--frozen_model_blr", type=float, default=-1, help="base lr bound for frozen model (default: %(default)s)"
    )
    parser.add_argument(
        "--scheduler",
        type=str,
    ...
    parser.add_argument(
        "--warmup_steps", type=int, default=-1, help="Steps to warmup LR, if scheduler supports (default: %(default)s)"
    )
    parser.add_argument(
        "--warmup_tokens",
        type=int,
    ...
    parser.add_argument(
        "--fixed_eval_input_tokens", default=128, type=int, help="Number of input tokens for the fixed evaluation"
    )
    parser.add_argument(
        "--fixed_eval_target_tokens", default=128, type=int, help="Number of target tokens for the fixed evaluation"
    )
    parser.add_argument("--fixed_eval_batch_size", default=32, type=int, help="Batch size for the fixed evaluation")
    ...
    parser.add_argument(
        "--rlimit", default=4096, type=int, help='Increase rlimit to avoid "RuntimeError: received 0 items of ancdata".'
    )
    ...
    parser.add_argument("--wandb_run_name", default="auto", type=str, help="Run name on wandb")
...
def setup_modality_info(args):
    ...
    for mod in modality_info:
        image_size, patch_size = modality_info[mod].get("input_size", args.input_size), modality_info[mod].get(
...
def setup_data(args):
    ...
    if any([cfg["data_path"].startswith("s3") for cfg in train_config.values()]):
        utils.s3_utils.override_wds_s3_tar_loading(
            args.s3_data_endpoint, args.s3_multipart_threshold_mb, args.s3_multipart_chunksize_mb, args.s3_max_io_queue
        )
    ...
    if "val" in data_config:
        ...
        for dataset_name, dataset_cfg in val_config.items():
            ...
            data_loaders_val[dataset_name] = get_val_dataloader(
                ...
                min_target_tokens=args.min_target_tokens,
                fixed_eval=False,
                fixed_eval_input_tokens=args.fixed_eval_input_tokens,
                fixed_eval_target_tokens=args.fixed_eval_target_tokens,
...
def get_model(args, modality_info):
    ...
    for mod in args.in_domains:
        ...
        if info.get("encoder_embedding", None) is not None:
            if info["type"] == "img":
                image_size, patch_size = info.get("input_size", args.input_size), info.get(
                    "patch_size", args.patch_size
                )
...
def main(args):
    ...
    if args.epochs < 0:
        if args.total_tokens < 0:
            ...
            args.epochs = math.ceil(
                args.total_tokens * 1e9 / ((args.num_input_tokens + args.num_target_tokens) * train_dataset_size)
            )
    ...
    if args.warmup_epochs < 0 and args.warmup_steps < 0:
        if args.warmup_tokens < 0:
            ...
            args.warmup_steps = math.ceil(
                args.warmup_tokens
                * 1e9
                / ((args.num_input_tokens + args.num_target_tokens) * args.batch_size * utils.get_world_size())
            )
    ...
    if args.cooldown_epochs < 0 and args.cooldown_steps < 0:
        if args.cooldown_tokens < 0 and args.lr_schedule in ["inverse_sqrt"]:
            ...
            args.cooldown_steps = math.ceil(
                args.cooldown_tokens
                * 1e9
                / ((args.num_input_tokens + args.num_target_tokens) * args.batch_size * utils.get_world_size())
            )
    ...
    if args.frozen_model_epochs <= 0:
        if args.frozen_model_tokens > 0:
            train_dataset_size = args.epoch_size  # or len(dataset_train)
            args.frozen_model_epochs = math.ceil(
                args.frozen_model_tokens * 1e9 / ((args.num_input_tokens + args.num_target_tokens) * train_dataset_size)
            )
        else:
    ...
    if args.frozen_model_epochs > 0:
        frozen_lr_schedule_values = utils.constant_scheduler(
            args.frozen_model_lr, args.frozen_model_epochs, num_training_steps_per_epoch
        )
        frozen_wd_schedule_values = utils.constant_scheduler(
            args.weight_decay, args.frozen_model_epochs, num_training_steps_per_epoch
        )
    ...
    fsdp_utils.auto_load_model_fsdp(args=args, model=model, optimizer=optimizer)
    ...
    if args.eval:
        ...
        if data_loaders_fixed_eval is not None:
            for dataset_name, data_loader_fixed_eval in data_loaders_fixed_eval.items():
                ...
                fixed_eval_stats = evaluate(
                    model,
                    data_loader_fixed_eval,
                    device,
    ...
    for epoch in range(args.start_epoch, args.epochs):
            ...
            if (epoch + 1) % args.save_ckpt_freq == 0 or epoch + 1 == args.epochs:
                ...
                if epoch + 1 == args.epochs:
                    ...
                    fsdp_utils.save_model_fsdp(
                        args=args, model=model, optimizer=optimizer, epoch=epoch, ckpt_name="final", use_s3=use_s3
                    )
        ...
        log_stats = {
            **{k: v for k, v in train_stats.items()},
            ...
            "input_tokens_seen_b": (epoch + 1)
            * num_training_steps_per_epoch
            * (total_batch_size / args.accum_iter)
            * args.num_input_tokens
            / 1e9,
            "target_tokens_seen_b": (epoch + 1)
            * num_training_steps_per_epoch
            * (total_batch_size / args.accum_iter)
            * args.num_target_tokens
            / 1e9,
            "total_tokens_seen_b": (epoch + 1)
            * num_training_steps_per_epoch
            * (total_batch_size / args.accum_iter)
            * (args.num_input_tokens + args.num_target_tokens)
            / 1e9,
        }
                ...
                log_writer.update(
                    {
                        ...
                        "total_tokens_seen_b": it
                        * (total_batch_size / accum_iter)
...
def evaluate(
    ...
    for x in metric_logger.log_every(data_loader, print_freq, iter_len=iter_len, header=header):
        ...
        with torch.cuda.amp.autocast(dtype=dtype, enabled=dtype != torch.float32):
            loss, mod_loss = model(
                mod_dict, num_encoder_tokens=num_input_tokens, num_decoder_tokens=num_target_tokens, loss_type=loss_type
            )

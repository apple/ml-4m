...
def get_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--input_size_sd", default=512, type=int, help="Image size used for stabel diffusion (default: %(default)s)"
    )
    parser.add_argument(
        "--max_input_size",
        default=False,
    ...
    parser.add_argument("--freeze_enc", action="store_true", help="Freeze encoder and quantizer (default: %(default)s)")
    parser.add_argument("--no_freeze_enc", action="store_false", dest="freeze_enc")
    parser.set_defaults(freeze_enc=False)
    parser.add_argument(
        "--dec_transformer_dropout",
        default=0.2,
        type=int,
    ...
    parser.add_argument("--guidance_scale", default=2.5, type=float, help="Guidance scale (default: %(default)s)")
    parser.add_argument(
        "--cond_scale",
        default=0.8,
        type=float,
    ...
    parser.add_argument(
        "--dynamic_guidance", action="store_true", default=False, help="Enable dynamic guidance (default: %(default)s)"
    )
    parser.add_argument(
        "--masked_cfg",
        action="store_true",
    ...
    parser.add_argument(
        "--adapter", default=None, type=str, help="Path to adapter pretrained weights (default: %(default)s)"
    )
...
def main(args):
    ...
    if args.max_input_size:
        args.input_size = max(
            args.input_size_max, args.input_size_sd
        )  # Load the largest resolution and downsample accordingly
    else:
        args.input_size = (
            args.input_size_max
    ...
    image_augmenter_train = RandomCropImageAugmenter(
        target_size=args.input_size, main_domain=args.domain, crop_scale=(args.min_crop_scale, 1.0)
    )

    MODALITY_TRANSFORMS_VQCONTROLNET["rgb"] = RGBTransform(
    ...
    if args.use_wds:
        ...
        data_loader_train = build_wds_divae_dataloader(
            ...
            modality_transforms=MODALITY_TRANSFORMS_VQCONTROLNET,
        ...
        transforms_train = UnifiedDataTransform(
            transforms_dict=MODALITY_TRANSFORMS_VQCONTROLNET, image_augmenter=image_augmenter_train
        )
    ...
    if args.eval_data_path:
        ...
        transforms_val = UnifiedDataTransform(
            transforms_dict=MODALITY_TRANSFORMS_VQCONTROLNET, image_augmenter=image_augmenter_val
        )
...
def train_one_epoch(
    ...
    eval_freq=1000,
    eval_metrics_freq=10_000,
    eval_image_log_freq=10_000,
    ...
    for step, x in enumerate(metric_logger.log_every(data_loader, print_freq, iter_len=loader_len, header=header)):
        ...
        if step_eval:
            ...
            launch_eval_image_log = (
                (data_loader_image_log is not None) and (it % eval_image_log_freq == 0) and (it != 0)
            )
            ...
            eval_stats = launch_evals(
                ...
                eval_image_sizes=eval_image_sizes,
                eval_noise_schedule=eval_noise_schedule,
                num_eval_timesteps=num_eval_timesteps,
                model_ema=model_ema,
                dtype=dtype,
                data_loader_val=data_loader_val,
                data_loader_metrics=data_loader_metrics,
        ...
        clean_input = unwrap_model(model).vae_encode(
            F.interpolate(clean_images, args.input_size_sd, mode="bilinear", align_corners=False)
        )

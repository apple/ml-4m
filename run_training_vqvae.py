...
from fourm.data import CenterCropImageAugmenter, RandomCropImageAugmenter, NoImageAugmenter, build_wds_divae_dataloader
from fourm.data.modality_transforms import UnifiedDataTransform, RGBTransform, NormalTransform, SAMInstanceTransform
...
try:
    ...
    print(e)
    print(
        "ImageBind utils are not found, if you'd like to train an ImageBind tokenizer, please install them following the official instructions at https://github.com/facebookresearch/ImageBind . Otherwise, you can ignore this warning."
    )
...
def get_args() -> argparse.Namespace:
    ...
    # Model parameters
    parser.add_argument("--patch_size", default=16, type=int, help="Patch size for ViT encoder (default: %(default)s)")
    parser.add_argument("--input_size_min", default=224, type=int, help="Minimum image size (default: %(default)s)")
    parser.add_argument("--input_size_max", default=512, type=int, help="Maximum image size (default: %(default)s)")
    parser.add_argument(
        "--resolution_step",
        default=32,
        type=int,
    ...
    parser.add_argument(
        "--encoder_type", default="vit_s_enc", type=str, metavar="ENC", help="Name of encoder (default: %(default)s)"
    )
    parser.add_argument(
        "--decoder_type", default="vit_b_dec", type=str, metavar="DEC", help="Name of decoder (default: %(default)s)"
    )
    ...
    parser.add_argument(
        "--encoder_ckpt", default=None, type=str, help="Optional path to encoder checkpoint (default: %(default)s)"
    )
    parser.add_argument(
        "--full_ckpt",
        default=None,
    ...
    parser.add_argument(
        "--loss_fn", default="mse", type=str, help="Reconstruction loss function (default: %(default)s)"
    )
    parser.add_argument(
        "--percept_loss_type",
        default=None,
    ...
    parser.add_argument(
        "--num_codebooks", default=1, type=int, help="Number of quantizer codebooks (default: %(default)s)"
    )
    parser.add_argument(
        "--latent_dim",
        default=32,
    ...
    parser.add_argument(
        "--codebook_weight", default=1.0, type=float, help="Weight of code book loss (default: %(default)s)"
    )
    parser.add_argument(
        "--quantizer_ema_decay", default=0.8, type=float, help="Quantizer EMA decay rate (default: %(default)s)"
    )
    parser.add_argument(
        "--coef_ema_dead_code", default=4.0, type=float, help="Dead code restart coefficient (default: %(default)s)"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--batch_size_eval", default=None, type=int, help="Batch size per GPU during evaluation (default: %(default)s)"
    )
    parser.add_argument("--epochs", default=100, type=int, help="Number of epochs (default: %(default)s)")
    parser.add_argument(
        "--save_ckpt_freq", default=10, type=int, help="Checkpoint saving frequency in epochs (default: %(default)s)"
    )
    parser.add_argument(
        "--opt", default="adamw", type=str, metavar="OPTIMIZER", help="Optimizer (default: %(default)s)"
    )
    parser.add_argument(
        "--opt_eps", default=1e-8, type=float, metavar="EPSILON", help="Optimizer epsilon (default: %(default)s)"
    )
    parser.add_argument(
        "--opt_betas",
        default=[0.9, 0.95],
    ...
    parser.add_argument(
        "--clip_grad", type=float, default=None, metavar="CLIPNORM", help="Clip gradient norm (default: %(default)s)"
    )
    parser.add_argument(
        "--skip_grad",
        type=float,
    ...
    parser.add_argument(
        "--warmup_lr", type=float, default=1e-6, metavar="LR", help="Warmup learning rate (default: %(default)s)"
    )
    parser.add_argument(
        "--min_lr",
        type=float,
    ...
    parser.add_argument(
        "--hflip", type=float, default=0.5, help="Probability of horizontal flip (default: %(default)s)"
    )

    # Dataset parameters
    parser.add_argument("--domain", default="rgb", type=str, help="Domain/Task name to load (default: %(default)s)")
    parser.add_argument(
        "--mask_value",
        default=None,
        type=float,
    ...
    parser.add_argument(
        "--wds_n_repeats", default=1, type=int, help="Number of repeats for webdataset loader to improve efficiency"
    )
    parser.add_argument(
        "--wds_shuffle_buffer_tar", default=1_000, type=int, help="Webdatasets shuffle buffer after loading tar files"
    )
    parser.add_argument(
        "--wds_shuffle_buffer_repeat",
        default=1_000,
        type=int,
        help="Webdatasets shuffle buffer after repeating samples",
    )
    ...
    parser.add_argument(
        "--no_dist_eval", action="store_false", dest="dist_eval", help="Disabling distributed evaluation"
    )
    ...
    parser.add_argument(
        "--eval_metrics_freq", default=1, type=int, help="frequency of evaluation metrics (in iterations or epochs)"
    )
    parser.add_argument(
        "--eval_image_log_freq", default=5, type=int, help="frequency of evaluation image logging (in iterations)"
    )
    parser.add_argument("--num_logged_images", default=100, type=int, help="number of images to log")
    parser.add_argument("--eval_only", action="store_true", default=False)
    parser.add_argument(
        "--no_inception", action="store_true", default=False, help="Disable Inception metric during eval"
    )
    parser.add_argument("--log_codebook_usage", action="store_true", help="Log the codebook usage")
    parser.add_argument(
        "--no_codebook_usage",
        action="store_false",
        dest="log_codebook_usage",
        help="Disable logging of the codebook usage",
    )
    ...
    # Wandb logging
    parser.add_argument(
        "--log_wandb", default=False, action="store_true", help="Log training and validation metrics to wandb"
    )
...
def get_model(args: argparse.Namespace, device: Union[torch.device, str]) -> VQVAE:
    ...
    codebook_size_int = (
        np.prod([int(d) for d in args.codebook_size.split("-")])
        if isinstance(args.codebook_size, str)
...
def main(args: argparse.Namespace) -> None:
    ...
    if "human_poses" not in args.domain:
        image_augmenter_train = RandomCropImageAugmenter(
            target_size=args.input_size, main_domain=args.all_domains[0], crop_scale=(args.min_crop_scale, 1.0)
        )
    else:
    ...
    if args.use_wds:
        ...
        data_loader_train = build_wds_divae_dataloader(
            ...
            epoch_size=args.dataset_size,
            shuffle_buffer_load=args.wds_shuffle_buffer_tar,
            shuffle_buffer_repeat=args.wds_shuffle_buffer_repeat,
            n_repeats=args.wds_n_repeats,
        )

        num_training_steps_per_epoch = args.dataset_size // (args.batch_size * num_tasks)
    else:
        transforms_train = UnifiedDataTransform(
            transforms_dict=MODALITY_TRANSFORMS_DIVAE, image_augmenter=image_augmenter_train
        )
    ...
    if args.eval_data_path:
        ...
        transforms_val = UnifiedDataTransform(
            transforms_dict=MODALITY_TRANSFORMS_DIVAE_VAL, image_augmenter=image_augmenter_val
        )
    ...
    train_res_choices = list(
        range(args.input_size_min, args.input_size_max + args.resolution_step, args.resolution_step)
    )
    ...
    if args.eval_only:
        ...
        eval_stats = evaluate(
            ...
            train_res_choices,
            args.loss_fn,
            args.codebook_weight,
            args.percept_loss_weight,
            dtype=dtype,
...
def mask_out_samples(
    clean_inputs: torch.Tensor, mask_valid: Optional[torch.BoolTensor] = None, mask_value: Optional[float] = None
) -> torch.Tensor:
...
def compute_reconst_loss(model_output: torch.Tensor, images: torch.Tensor, loss_fn: str) -> torch.Tensor:
    ...
    if loss_fn == "mse":
        ...
        reconst_loss = (
            1
            - F.cosine_similarity(
                rearrange(model_output, "b c h w -> b c (h w)"), rearrange(images, "b c h w -> b c (h w)"), dim=-1
            ).mean()
        )
...
def train_one_epoch(
    ...
    for step, x in enumerate(metric_logger.log_every(data_loader, print_freq, iter_len=loader_len, header=header)):
        ...
        if step_eval:
            ...
            launch_eval_metrics = (
                (data_loader_metrics is not None)
                and eval_metrics_freq > 0
                and (it % eval_metrics_freq == 0)
                and (it != 0)
            )
            launch_eval_image_log = (
                (data_loader_image_log is not None)
                and eval_image_log_freq > 0
                and (it % eval_image_log_freq == 0)
                and (it != 0)
            )
            ...
            eval_stats = launch_evals(
                ...
                percept_loss_weight=percept_loss_weight,
                train_res_choices=train_res_choices,
                eval_image_sizes=eval_image_sizes,
                model_ema=model_ema,
                dtype=dtype,
                ...
                log_writer=log_writer,
                log_codebook_usage=log_codebook_usage,
            )
        ...
        with torch.cuda.amp.autocast(dtype=dtype_percept, enabled=dtype_percept != torch.float32):
            if percept_loss_fn is not None and percept_loss_weight > 0:
                ...
                percept_loss = percept_loss_fn(
                    transform(model_output).to(dtype_percept), transform(images).to(dtype_percept)
                ).mean()
            else:
...
def launch_evals(
    ...
    if launch_evaluate:
        ...
        if model_ema is not None:
            eval_stats = evaluate(
                unwrap_model(model_ema),
                unwrap_model(feature_extractor),
                percept_loss_fn,
                data_loader_val,
                device,
                domain,
...
def eval_metrics(
    ...
    if log_codebook_usage:
        ...
        if utils.is_main_process() and log_writer is not None:
            ...
            if hasattr(unwrap_model(model).quantize, "_codebook") and hasattr(
                unwrap_model(model).quantize._codebook, "cluster_size"
            ):
    ...
    if utils.is_main_process():
        ...
        for x in data_loader:
            ...
            if domain in ["rgb", "normal"]:
                ...
                gt_onehot = rearrange(F.one_hot(images, num_classes=num_classes), "b h w c -> b c h w").float()
                reconst_onehot = rearrange(
                    F.one_hot(output.argmax(dim=1), num_classes=num_classes), "b h w c -> b c h w"
                ).float()
        ...
        if len(gt_imgs) > 0:
            log_writer.wandb_safe_log(
                {f"{prefix} GT @{eval_size}": wandb.Image(make_grid(gt_imgs[:num_logged_images]))}, commit=False
            )
            log_writer.wandb_safe_log(
                {f"{prefix} Reconst @{eval_size}": wandb.Image(make_grid(reconst_imgs[:num_logged_images]))},
                commit=False,
            )

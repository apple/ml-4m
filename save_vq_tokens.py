...
class SaveVQDataset(Dataset):
    def __init__(
        ...
        self.center_crop_augmenter = CenterCropImageAugmenter(target_size=self.input_size, hflip=0.0, main_domain=task)
    ...
    def __getitem__(self, index):
        ...
        for i, j, h, w, h_flip in settings:
            ...
            img_mod = self.task_transforms[self.task].image_augment(
                img_mod, (i, j, h, w), h_flip, None, (self.input_size, self.input_size), None, self.resample_mode
            )
            ...
            if self.mask_value is not None:
                ...
                mask_valid = self.task_transforms["mask_valid"].image_augment(
                    mask_valid, (i, j, h, w), h_flip, None, (self.input_size, self.input_size), None, None
                )
...
def main(args):
    ...
    data_loader = torch.utils.data.DataLoader(
        dataset, sampler=sampler, batch_size=args.batch_size_dataloader, num_workers=args.num_workers, drop_last=False
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--tokenizer_id", type=str, default="cc12m/rgb_ViTB-UNetP4_16k_224-448", help="ID of tokenizer to load."
    )
    parser.add_argument(
        "--tokenizers_root", type=str, default="./tokenizer_ckpts", help="Path where tokenizer checkpoints are saved."
    )
    parser.add_argument("--data_root", type=str, default="/path/to/dataset", help="Path to dataset root")
    ...
    parser.add_argument("--min_crop_scale", type=float, default=0.8, help="Minimum crop scale (Only for n_crops > 1)")
    ...
    parser.add_argument("--verbose", action="store_true", default=False, help="Set to enable progress bar")
    parser.add_argument(
        "--dryrun",
        action="store_true",
        default=False,
    ...
    parser.add_argument(
        "--batch_size_dataloader", default=64, type=int, help="Dataloader batch size (default: %(default)s)"
    )
    parser.add_argument("--batch_size", default=64, type=int, help="Batch size per GPU (default: %(default)s)")
    ...
    parser.add_argument(
        "--force_load_crop", action="store_true", help="Make sure to load crops locally, otherwise break the code."
    )
